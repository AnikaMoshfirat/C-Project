...
namespace Barber_Shop_Management_System
{
    public partial class Billing : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            // Get the next BillingID from the database
            int BillingID = GetNextBillingID();

            // Insert the data into the Billing table using a parameterized query
            string query = "INSERT INTO Billing (BillingID, TotalBill, CustomerID, BillingDate, CategoryID, ProductID) VALUES (@BillingID, @TotalBill, @CustomerID, @BillingDate, @CategoryID, @ProductID)";
        ...
        // Method to get the next available BillingID
        private int GetNextBillingID()
        {
            int newBillingID = 1; // Default to 1 if the table is empty

            string query = "SELECT MAX(BillingID) FROM Billing"; // Query to get the max BillingID
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    {
                        newBillingID = Convert.ToInt32(result) + 1; // Increment the latest BillingID
                    }
                }
            }
