...
namespace Barber_Shop_Management_System
{
            ...
            var allTimes = new List<string>
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            // Validate inputs
            if (string.IsNullOrWhiteSpace(CustomerID) || string.IsNullOrWhiteSpace(AppointmentDate) ||
            ...
            // Get the latest AppointmentID from the database
            int AppointmentID = GetNextAppointmentID();

            // Insert new appointment into database
            string query = @"INSERT INTO Appointment (AppointmentID, ServiceName, CategoryName, AppointmentDate, AppointmentTime, CustomerID)
                     VALUES (@AppointmentID, @ServiceName, @CategoryName, @AppointmentDate, @AppointmentTime, @CustomerID)";
        ...
        // Method to get the next available AppointmentID
        private int GetNextAppointmentID()
        {
            int newAppointmentID = 1; // Default to 1 if the table is empty

            string query = "SELECT MAX(AppointmentID) FROM Appointment"; // Query to get the max AppointmentID
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    {
                        newAppointmentID = Convert.ToInt32(result) + 1; // Increment the latest AppointmentID
                    }
                }
            }
